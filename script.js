...
var roadnetFile = /roadnetFile=([^\&]*)/.exec(location.search);
if (roadnetFile) {
    roadnetFile = roadnetFile[1];
} else {
    alert("no roadnetFile!");
    throw new Error("no roadnetFile!");
}
var logFile = /logFile=([^\&]*)/.exec(location.search);
if (logFile) {
    logFile = logFile[1];
} else {
    alert("no logFile!");
